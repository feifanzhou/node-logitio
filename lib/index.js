...
var LOGIT_URI = 'https://api.logit.io/node/v1';
...
var Logit = function() {
     ...
     *
     */
    this.init = function(apiKey, options) {
        if (!apiKey) {
            console.warn('API key for Logit endpoint must be specified as second parameter when calling window.logit.init()');
            return;
        }
        this.apiKey = apiKey;
        ...
        // Set verbosity
        if( this.options.verbosity ) this.verbosity = this.options.verbosity;
    ...
    this.createMessage = function (priority, message, dimensions, opts) {
        if (!this.initialised) {
            console.warn('Logit.io plugin not initialised\n\nCall window.logit.init() first.');
            return;
        }
        ...
        if (this.options.logToConsole) {
            ...
            var msg = pkg.message;
            var t = pkg.timestamp;
            var details = JSON.stringify(pkg.properties);
            consoleFn.call(console, this.getPriorityName(priority) + ': ' + t + ' - ' + msg + '; ' + details);
        }
    };
    ...
    function checkAndSend() {
        if (!this.disableSending && this.queue && !this.sending) {
            this._intervalSend();
        }
    }
};
